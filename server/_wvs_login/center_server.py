...
class CenterServer:
    ...
    async def create_connection(self):
        ...
        with Packet(op_code=InterOps.RegistrationRequest) as out_packet:
            ...
            out_packet.encode_byte(WORLD_COUNT)
            
            for world in self._parent._worlds:
                out_packet.encode_byte(world._id)
                out_packet.encode_string(world._name)
                out_packet.encode_short(world._port)
                out_packet.encode_byte(1)
                out_packet.encode_string(world._ticker_message)
                out_packet.encode_byte(world._allow_multi_leveling)
                out_packet.encode_int(world._exp_rate)
                out_packet.encode_int(world._quest_exp_rate)
                out_packet.encode_int(world._party_quest_exp_rate)
                out_packet.encode_int(world._meso_rate)
                out_packet.encode_int(world._drop_rate)
