...
class FastAes:
    ...
    def transform_block(self, block):
        ...
        while cur_round < (self.__rounds - 2):
            ...
            r1 = self.table0[C1 & 255] ^ self.table1[(C2 >> 8) & 255] ^ self.table2[(C3 >> 16) & 255] ^ self.table3[C0 >> 24] ^ wk[1]
            r2 = self.table0[C2 & 255] ^ self.table1[(C3 >> 8) & 255] ^ self.table2[(C0 >> 16) & 255] ^ self.table3[C1 >> 24] ^ wk[2]
            r3 = self.table0[C3 & 255] ^ self.table1[(C0 >> 8) & 255] ^ self.table2[(C1 >> 16) & 255] ^ self.table3[C2 >> 24] ^ wk[3]
            cur_round += 1
            wk = self._working_key[cur_round]
            C0 = self.table0[r0 & 255] ^ self.table1[(r1 >> 8) & 255] ^ self.table2[(r2 >> 16) & 255] ^ self.table3[r3 >> 24] ^ wk[0]
            C1 = self.table0[r1 & 255] ^ self.table1[(r2 >> 8) & 255] ^ self.table2[(r3 >> 16) & 255] ^ self.table3[r0 >> 24] ^ wk[1]
            C2 = self.table0[r2 & 255] ^ self.table1[(r3 >> 8) & 255] ^ self.table2[(r0 >> 16) & 255] ^ self.table3[r1 >> 24] ^ wk[2]
            C3 = self.table0[r3 & 255] ^ self.table1[(r0 >> 8) & 255] ^ self.table2[(r1 >> 16) & 255] ^ self.table3[r2 >> 24] ^ wk[3]
        ...
        block[0:4] = (C0).to_bytes(4, 'little')
        block[4:8] = (C1).to_bytes(4, 'little')
        block[8:12] = (C2).to_bytes(4, 'little')
        block[12:16] = (C3).to_bytes(4, 'little')

    @staticmethod
    def le_to_int32(buffer, offset):
    ...
    @staticmethod
    def sub_word(x):
        return FastAes.sbox[x & 255] \
            | FastAes.sbox[(x >> 8) & 255] << 8 \
            | FastAes.sbox[(x >> 16) & 255] << 16 \
            | FastAes.sbox[(x >> 24) & 255] << 24
